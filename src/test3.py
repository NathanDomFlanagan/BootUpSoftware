...
class ConfigManager(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.cat_menu = tk.OptionMenu(self, self.cat_var, ())
        ...
        tk.Button(btn_frame, text="New Category", command=self.new_category).grid(row=0, column=0, padx=5)
        ...
        tk.Button(btn_frame, text="Run Apps",      command=self.run_apps).grid(row=0, column=3, padx=5)
        tk.Button(btn_frame, text="Save & Exit",   command=self.on_close).grid(row=0, column=4, padx=5)
    ...
    def refresh_categories(self):
        ...
        if self.cfg:
            self.cat_var.set(next(iter(self.cfg)))
